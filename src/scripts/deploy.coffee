...
deploy = (msg, target) ->
  ...
  if target_list[target]?
    for host_name in target_list[target]
      ...
      runchef = spawn('ssh', ["-o 'StrictHostKeyChecking=no' #{process.env.ROUTER_USER}@#{process.env.ROUTER_IP}", "ssh -o 'StrictHostKeyChecking=no' #{process.env.DEPLOY_USER}@#{host_address} /home/ubuntu/run-chef.sh"])
